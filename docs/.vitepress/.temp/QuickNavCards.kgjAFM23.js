...
const _sfc_main = {
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "quick-nav-grid" }, _attrs))} data-v-89c70e23><!--[-->`);
      ssrRenderList(__props.items, (item) => {
        ssrRenderVNode(_push, createVNode(resolveDynamicComponent(item.disabled ? "div" : "a"), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="quick-nav-card__top" data-v-89c70e23${_scopeId}>`);
              if (item.icon) {
                _push2(`<span class="quick-nav-card__icon" data-v-89c70e23${_scopeId}>${ssrInterpolate(item.icon)}</span>`);
              } else {
                _push2(`<!---->`);
              }
              if (item.badge) {
                _push2(`<span class="quick-nav-card__badge" data-v-89c70e23${_scopeId}>${ssrInterpolate(item.badge)}</span>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div><h3 data-v-89c70e23${_scopeId}>${ssrInterpolate(item.title)}</h3><p class="quick-nav-card__desc" data-v-89c70e23${_scopeId}>${ssrInterpolate(item.description)}</p>`);
              if (item.meta) {
                _push2(`<p class="quick-nav-card__meta" data-v-89c70e23${_scopeId}>${item.meta ?? ""}</p>`);
              } else {
                _push2(`<!---->`);
              }
            } else {
...
const QuickNavCards = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-89c70e23"]]);
export {
  QuickNavCards as Q
};
